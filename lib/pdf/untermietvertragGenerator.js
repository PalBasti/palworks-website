// lib/pdf/untermietvertragGenerator.js - Mit Layout Manager Integration
...
// Übergabeprotokoll mit Layout Manager
function generateHandoverProtocolPDF_WithLayout(pdf, parentLayout, formData) {
  ...
  try {
    ...
    layout.addText(`Adresse: ${formData.property_address || '[Adresse]'}`);
    layout.addText(`Wohnfläche: ${formData.property_size ? `ca. ${formData.property_size} qm` : '[Größe]'}`);
    layout.addSpacing(6);
    ...
    const cleaningOptions = [
      'Wohnung ist gereinigt und besenrein',
      'Wohnung muss noch gereinigt werden', 
      'Wohnung ist verschmutzt/unrein'
    ];
    
    cleaningOptions.forEach(option => {
      layout.addCheckbox(option);
    });
    ...
    const repairOptions = [
      'Alle erforderlichen Schönheitsreparaturen sind durchgeführt',
      'Schönheitsreparaturen teilweise durchgeführt',
      'Schönheitsreparaturen sind noch erforderlich'
    ];
    ...
    layout.addSignatureSection([
      { 
        label: 'Unterschrift Untervermieter',
        name: formData.sublessor_name || '[Name]'
      },
      { 
        label: 'Unterschrift Untermieter',
        name: formData.sublessee_name || '[Name]'
      }
    ]);
...
export function generateAllDocuments(formData, selectedAddons = []) {
  try {
    ...
    // Hauptvertrag
    const mainContract = generateUntermietvertragPDF(formData, []);
    documents.push({
      type: 'main_contract',
