...
export function generateHandoverProtocolPDF(formData) {
  ...
  try {
    ...
    // Neue Seite für Zustandsbeschreibung
    checkPageBreak(50);
    ...
    const contractCity = displayValue(formData.property_city);
    
    if (contractCity) {
      addText(`Ort, Datum: ${contractCity}, den ${currentDate}`, margin, 11, 'normal');
    } else {
    ...
    pdf.setFontSize(10);
    pdf.setFont('helvetica', 'normal');
    pdf.text('Untervermieter/Bevollmächtigter', margin, signatureY + 6);
