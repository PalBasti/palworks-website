...
/**
 * Update contract payment status - NEUE FUNKTION FÜR INTEGRATION
 * @param {string} contractId - Contract ID
 * @param {Object} paymentData - Payment status data
 * @returns {Promise<Object>} Updated contract
 */
export async function updateContractPaymentStatus(contractId, paymentData) {
  try {
    // Verwende Admin-Client für Payment-Updates
    const client = supabaseAdmin || supabase
    ...
    if (paymentData.payment_status === 'paid' && !paymentData.paid_at) {
      updateData.paid_at = new Date().toISOString()
    }
    
    // Automatisch completed_at setzen wenn status auf 'completed' gesetzt wird
    if (paymentData.status === 'completed' && !paymentData.completed_at) {
      updateData.completed_at = new Date().toISOString()
    }
    ...
    throw new Error(`Fehler beim Aktualisieren des Zahlungsstatus: ${error.message}`)
  }
}
