// components/UntermietvertragForm.js - FINALE VERSION MIT DUPLIKAT-FILTER
...
const getContractAddons = async (contractType) => {
  try {
    ...
    // ✅ DUPLIKAT-FILTER: Entferne Addons mit gleichem Namen
    const uniqueAddons = data.filter((addon, index, array) => {
      return array.findIndex(a => a.name === addon.name) === index
    })
    
    console.log('🔍 Original addons loaded:', data.length)
    console.log('🔍 After duplicate removal:', uniqueAddons.length)
    console.log('🔍 Unique addon names:', uniqueAddons.map(a => a.name))
...
export default function UntermietvertragForm({ onSubmit }) {
  ...
  // ✅ Addons laden mit verbessertem Logging
  useEffect(() => {
    const loadAddons = async () => {
      try {
        console.log('🔍 Starting to load addons for:', 'untermietvertrag')
        const addonData = await getContractAddons('untermietvertrag')
        console.log('🔍 Final addons to be displayed:', addonData)
        setAddons(addonData)
      } catch (error) {
